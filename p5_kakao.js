...
try {
  const places = await searchPlace(query); //길이가 3인 배열로 반환됨
  if (places.length === 0) throw new Error(`No place found for: ${query}`); //제대로 실행 안 된 경우

  // TODO: 후보마다 한 줄: `${i + 1}. ${name}  ${address}  (${latitude.toFixed(4)}, ${longitude.toFixed(4)})`
  for(let i=0; i<places.length; i++){
    console.log(`${i + 1}. ${places[i].name}  ${places[i].address}  (${places[i].latitude.toFixed(4)}, ${places[i].longitude.toFixed(4)})`);
  }
